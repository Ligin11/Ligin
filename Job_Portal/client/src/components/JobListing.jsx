...
const JobListing = () => {
    ...
    const {isSearched, searchFilter, setSearchFilter, jobs} = useContext(AppContext)
     ...
     const [fileteredJobs, setFilteredJobs] = useState(jobs)
    ...
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4 '>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 '>
            {fileteredJobs.slice((currentPage-1)*6,currentPage*6).map((job,index)=>(
                <JobCard key={index} job={job}/>
            ))}
        </div>

        {/* Pagination */}
        {fileteredJobs.length > 0  && (
        <div className='flex items-center justify-center space-x-2 mt-10'>
            <a href="#job-list">
                <img onClick={()=> setCurrentPage(Math.max(currentPage-1),1)} src={assets.left_arrow_icon}/>
            </a>
            {Array.from({length:Math.ceil(fileteredJobs.length/6)}).map((_,index)=>(
            ...
            <a href="#job-list">
                <img onClick={()=> setCurrentPage(Math.min(currentPage+1, Math.ceil(fileteredJobs.length/6)))} src={assets.right_arrow_icon}/>
            </a>
        </div>
    )}
      </section>
